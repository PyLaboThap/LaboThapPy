...
# mettre toute les correlations disponibles?
...
def void_fraction_bankoff(P, x, rho_l, rho_v):
    ...
    factor K, using Bankoff's "variable density single-fluid" model. Belongs
    to the same K*alpha_h family as Hughmark and Armand-Treschev, but here K
    depends only on system pressure rather than quality or mass flux -
    reflecting Bankoff's physical argument that phase distribution
    (bubble concentration profile) is primarily a pressure-driven effect.

    Formulation:
        alpha_h = homogeneous void fraction (mass-flux-weighted)
    ...
    - Developed for steam-water flow; the model's entrainment/distribution
      ...
      void fraction data, with Bankoff claiming validity across all
      pressures, mass velocities, flow regimes and enthalpy changes,
      predicting most data within about +-10 percent - a broad claim from
      the original author, not a range independently reconfirmed by every
      later study.
    - As a K*alpha_h correlation, it shares the same general family as
      Hughmark (1962) and Armand-Massena, and is commonly used as a
      simple baseline for comparison against those.
    - No hydraulic diameter, mass flux or fluid-property dependence is
      included, so it doesn't capture geometry or flow-regime transition
      effects the way Hughmark's Z-parameter does - useful as a fast
      first-pass estimate, less reliable near regime boundaries
      (e.g., transition to annular or stratified flow).
    - Since it was fit to steam-water data, applying it to refrigerants
      (e.g., R290) is an extrapolation beyond the original working fluid.
...
#======================================================================
# ROUHANI-AXELSSON (Drift-flux correlations)
#======================================================================
...
def void_fraction_rouhani_axelsson(x, rho_l, rho_g, G, sigma):
    ...
    C0 = 1 + 0.2 * (1 - x)
    V_drift = 1.18 * ((G_GRAVITY * sigma * (rho_l - rho_g)) / (rho_l ** 2)) ** 0.25
...
#======================================================================
# DIX / CHEXAL (Drift-flux correlations)
#======================================================================
...
def void_fraction_dix(x, rho_l, rho_g, G, sigma):
    ...
    V_drift = 2.9 * ((G_GRAVITY * sigma * (rho_l - rho_g)) / (rho_l ** 2)) ** 0.25
...
#======================================================================
# WOLDESEMAYAT-GHAJAR (Drift-flux correlations)
#======================================================================

def void_fraction_woldesemayat_ghajar(x, rho_l, rho_g, G, sigma, D, P, theta=np.pi/2):
    """
    Woldesemayat and Ghajar (2007).
    ...
    V_drift = (2.9 * ((G_GRAVITY * sigma * D * (1 + np.cos(theta)) * (rho_l - rho_g)) / (rho_l ** 2)) ** 0.25
...
def compute_void_fraction(AS, params, void_fraction_model='Homogeneous'):
    """
    ...
    void_fraction_model : str
        ...
        'Lockhart-Martinelli-v1', 'Lockhart-Martinelli-v2', 'Hughmark',
    ...
    if void_fraction_model == 'Homogeneous':
        ...
        alpha = void_fraction_bankoff(P, x, rho_l, rho_v)

    elif void_fraction_model == 'Rouhani-Axelsson':
        G = params.get('G')
        sigma = params.get('sigma')
        alpha = void_fraction_rouhani_axelsson(x, rho_l, rho_v, G, sigma)

    elif void_fraction_model == 'DIX':
        G = params.get('G')
        sigma = params.get('sigma')
        alpha = void_fraction_dix(x, rho_l, rho_v, G, sigma)
        ...
        alpha = void_fraction_woldesemayat_ghajar(x, rho_l, rho_v, G, sigma, D, P, theta)

    elif void_fraction_model == 'Cioncolini-Thome':
        alpha = void_fraction_cioncolini_thome(x, rho_l, rho_v)
