...
GAMMA = 1.4
R_AIR = 285.71429  # J/(kg*K)

OIL_T = np.array([100,110,120,130,140,150,160,170,180,190,200,210,
                  220,230,240,250,260,270,280,290,300,310,320,330,
                  340,350,360,370])  # °C
OIL_RHO = np.array([955,948,941,934,928,921,914,907,899,892,885,878,
                    870,863,856,848,840,832,825,817,809,800,792,783,
                    775,766,757,748])
OIL_CP  = np.array([1.84,1.87,1.91,1.94,1.98,2.01,2.05,2.09,2.12,2.16,
                    2.19,2.23,2.27,2.30,2.34,2.38,2.42,2.45,2.49,2.53,
                    2.57,2.61,2.65,2.69,2.73,2.77,2.81,2.85])*1e3
OIL_K   = np.array([0.1135,0.1128,0.1121,0.1114,0.1107,0.1099,0.1091,
                    0.1083,0.1074,0.1065,0.1056,0.1046,0.1036,0.1026,
                    0.1015,0.1004,0.0993,0.0982,0.0970,0.0958,0.0946,
                    0.0933,0.0920,0.0906,0.0893,0.0879,0.0865,0.0850])
OIL_MU  = np.array([3.60,2.92,2.42,2.05,1.75,1.52,1.33,1.18,1.06,0.95,
                    0.86,0.784,0.718,0.661,0.611,0.567,0.529,0.495,0.464,
                    0.437,0.413,0.391,0.371,0.353,0.336,0.321,0.308,0.295])*1e-3

C2_NL   = np.array([1,2,3,4,5,7,10,13,16])
C2_VAL  = np.array([0.7,0.8,0.86,0.9,0.92,0.95,0.97,0.98,0.99])

DEBUG_INLINE = False  # turn on only when needed


def oil_props(T_C):
    # T_C in °C
    T = np.clip(T_C, OIL_T[0], OIL_T[-1])
    rho = np.interp(T, OIL_T, OIL_RHO)
    cp  = np.interp(T, OIL_T, OIL_CP)
    k   = np.interp(T, OIL_T, OIL_K)
    mu  = np.interp(T, OIL_T, OIL_MU)
    Pr  = mu*cp/k
...
def fg_radiative_htc_corr():
    ...
    def make_bilinear_interpolator(x, y, Z):
        x = list(x)
        y = list(y)
        Z = [list(row) for row in Z]   # 2D list
        nx = len(x)
        ny = len(y)
    ...
    a_1_val = np.array([
        [0.082 , 0.1281, 0.1742, 0.2203, 0.2602],
    ...
    a_2_val = np.array([
        [-0.0304, -0.0347, -0.039 , -0.0433, -0.0651],
    ...
    a_3_val = np.array([
        [0.00076, 0.00238, 0.004  , 0.00562, -0.00155],
    ...
    f_0 = make_bilinear_interpolator(p_ratio_val, T_g_val, a_0_val)
    f_1 = make_bilinear_interpolator(p_ratio_val, T_g_val, a_1_val)
    f_2 = make_bilinear_interpolator(p_ratio_val, T_g_val, a_2_val)
    f_3 = make_bilinear_interpolator(p_ratio_val, T_g_val, a_3_val)
...
def external_flow_inline_bank(AS, T_in, T_out, T_w, P_in, u_in, N_col, D_out, S_T, S_L):
    """
    Crossflow over inline tube bank – convective HTC and DP per row.
    """
    ...
    # --- Fluid properties ---
    if fluid == 'Air':
        AS.update(CP.PT_INPUTS, P_in, T_m)
        rho = AS.rhomass()
        mu  = AS.viscosity()
        k   = AS.conductivity()
        Pr  = AS.Prandtl()

        AS.update(CP.PT_INPUTS, P_in, T_w)
        mu_w = AS.viscosity()
        k_w  = AS.conductivity()
        Pr_w = AS.Prandtl()

    elif fluid == 'Oil':
        rho, cp, k, mu, Pr = oil_props(T_in - 273.15)
        _,  cp_w, k_w, mu_w, Pr_w = oil_props(min(T_w - 273.15, 370.0))
        ...
        try:
            ...
            AS.update(CP.PT_INPUTS, P_in, T_w - 1.0)

        mu_w = AS.viscosity()
        k_w  = AS.conductivity()
        Pr_w = AS.Prandtl()

    # --- Reynolds, velocity, Mach check ---
    V_max = S_T / (S_T - D_out) * u_in
    Re_max = rho * V_max * D_out / mu

    a_gas = (GAMMA * R_AIR * T_in) ** 0.5
    if DEBUG_INLINE and V_max >= a_gas:
        print('Supersonic regime in the tubes: V_max =', V_max, '> a =', a_gas)

    if DEBUG_INLINE:
        if Re_max > 2e6:
            print('Re out of range (>2e6):', Re_max)
        elif Re_max < 30:
            print('Re out of range (<30):', Re_max)
        if Pr < 0.5 or Pr > 500:
            print('Prandtl out of range:', Pr)
        if b < 1.25 or b > 2.5:
            print('S_L/D out of range:', b)
    ...
    if 10 < Re_max < 1e2:
        ...
        Nu = (
            0.3
            + (0.62 * Re_max**0.5 * Pr**(1/3))
              / (1 + (0.4/Pr)**(2/3))**0.25
              * (1 + (Re_max/282000)**(5/8))**(4/5)
        )
        Pe = Pr * Re_max
        if DEBUG_INLINE and Pe <= 0.2:
            print('Peclet too small in External_Flow_Cylinder:', Pe)
        ...
        if S_T / S_L > 0.7:
            ...
            print('S_T/S_L < 0.7 – aligned tubes not recommended')

    elif 2e5 < Re_max < 2e6:
        C = 0.021
        m = 0.84

    # Banks correction / generic power-law Nu expression
    if C > 0.0:  # i.e. we are in a C,m regime
        if (N_col < 20) and (Re_max < 1e2 or Re_max > 1e3):
            C2 = float(np.interp(N_col, C2_NL, C2_VAL))
            Nu = C2 * C * Re_max**m * Pr**0.36 * (Pr/Pr_w)**0.25
        elif (N_col >= 20) and (Re_max < 1e2 or Re_max > 1e3):
            Nu = C * Re_max**m * Pr**0.36 * (Pr/Pr_w)**0.25

    # --- Pressure drop and HTC ---
    Eu = euler_coeff(a, b, Re_max)    # Euler number
    DP_row = 0.5 * Eu * rho * V_max**2
    h = Nu * k / D_out

    return h, DP_row, Nu, Re_max, V_max, a_gas


def external_flow_staggered_bank(AS, T_in, T_out, T_w, P_in, u_in, N_col, D_out, S_T, S_L):
    ...
    if AS.fluid_names()[0] == 'Air':
        ...
        rho = np.interp(T_in-273.15, T_f_val, rho_f_val)
        cp = np.interp(T_in-273.15, T_f_val, cp_f_val)
        k = np.interp(T_in-273.15, T_f_val, k_f_val)
        mu = np.interp(T_in-273.15, T_f_val, mu_f_val)
        Pr = (mu*cp)/k

        # Wall mean properties        
        if T_w-273.15 > 370:
            cp_w = np.interp(370, T_f_val, cp_f_val)
            k_w = np.interp(370, T_f_val, k_f_val)
            mu_w = np.interp(370, T_f_val, mu_f_val)
            Pr_w = (mu_w*cp_w)/k_w
        else:
            cp_w = np.interp(T_w-273.15, T_f_val, cp_f_val)
            k_w = np.interp(T_w-273.15, T_f_val, k_f_val)
            mu_w = np.interp(T_w-273.15, T_f_val, mu_f_val)
            Pr_w = (mu_w*cp_w)/k_w
    ...
    if (N_col<20) and ((Re_max<1e2) or (Re_max>1e3)):	# Apply a correction factor when there is less than 20 rows of tube as thee first rows influence significantly the flow on the following and thus influencing the Nusselt number
        ...
        C_2 = np.interp(N_col, N_L_val, C_2_val)
...
def pool_boiling(AS, T_sat, T_tube):
    ...
    # Liquid Properties
    AS.update(CP.QT_INPUTS, T_sat, 0)
    rho_l = AS.rhomass()
    mu_l = AS.viscosity()
    cp_l = AS.cpmass()
    h_l = AS.hmass()
    ...
    # 2 Phase Properties
    AS.update(CP.QT_INPUTS, T_sat, 0.5)
    sigma = AS.surface_tension() # surface tension of liquid vapor equilibrium
    Dh_evap = h_v - h_l # Evaporation specific heat
...
def film_boiling(D_out,AS,T_sat,T_tube,P_sat):
    """
    ---- Inputs : -------- 
    ...
    # Film thermal conductivity, density, kinematic viscosity, enthalpy and capacity
    AS.update(CP.PT_INPUTS, P_sat, T_f)
    rho_f = AS.rhomass()
    mu_f = AS.viscosity()
    cp_f = AS.cpmass()
    k_f = AS.conductivity()
    ...
    # Vapor Properties
    AS.update(CP.PT_INPUTS, T_sat, 1)
    h_v = AS.hmass()

    # Liquid Properties
    AS.update(CP.PT_INPUTS, T_sat, 0)
    h_l = AS.hmass()
    rho_l = AS.rhomass()
    ...
    Dh_evap = h_v - h_l
    Dh_evap_corrected = Dh_evap + 0.8*cp_f*(T_tube - T_sat) # Accounts for sensible heat to maintain temperature of the film above saturation
    
    coef = (g*(rho_l - rho_f)*Dh_evap_corrected*D_out**3)/(mu_f*k_f*(T_tube - T_sat))
    
    h_conv = (D_out/k_f)*C*coef**(1/4) # convection coefficient
...
def boiling_curve(D_out, AS, T_sat, P_sat):
    """
    ---- Inputs : -------- 
    ...
    # Vapor Properties
    AS.update(CP.PT_INPUTS, T_sat, 0.5)
    ...
    for i in range(len(DT)):
        T_surf = DT[i] + T_sat
        q_pool[i] = pool_boiling(AS, T_sat, T_surf)
        h_film = film_boiling(D_out, AS, T_sat, T_surf, P_sat)
...
def ext_conv_boil(D_out, AS, T_sat, T_tube, V_flow):
    """
    ---- Inputs : -------- 
    ...
    """
    # Liquid properties
    AS.update(CP.QT_INPUTS, 0, T_sat)
    rho_l = AS.rhomass()
    h_l = AS.hmass()

    # Vapor properties
    AS.update(CP.QT_INPUTS, 1, T_sat)
    rho_v = AS.rhomass()
    h_v = AS.hmass()

    # 2 Phase properties    
    AS.update(CP.QT_INPUTS, 0.5, T_sat)
    sigma = AS.surface_tension()
...
def ext_tube_film_condens(D_out, AS, T_sat, T_w, V_flow):
    """
    ---- Inputs : -------- 
