...
class HexCstEffDisc(BaseComponent):
    ...
    **Outputs**:
    ...
    def solve(self):
        ...
        self.AS_H = CP.AbstractState('BICUBIC&HEOS', hot_su.fluid)
        self.AS_C = CP.AbstractState('BICUBIC&HEOS', cold_su.fluid)
        ...
        self.p_hot[0] = hot_su.p
        self.p_cold[0] = cold_su.p
        ...
        self.update_connectors(cold_ex, cold_su, hot_ex, hot_su)
        self.solved = True
        return

    def update_connectors(self, cold_ex, cold_su, hot_ex, hot_su):
        
        "Mass Connectors"
        cold_ex.set_fluid(cold_su.fluid)
        cold_ex.set_m_dot(cold_su.m_dot)
        cold_ex.set_h(cold_su.h + self.Q/cold_su.m_dot)
        cold_ex.set_p(self.p_cold[-1])
        
        self.AS_C.update(CP.HmassP_INPUTS, cold_ex.h, cold_ex.p)
        cold_ex.set_T(self.AS_C.T())

        hot_ex.set_fluid(hot_su.fluid)
        hot_ex.set_m_dot(hot_su.m_dot)
        hot_ex.set_h(hot_su.h - self.Q/hot_su.m_dot)
        hot_ex.set_p(self.p_hot[-1])
        
        self.AS_H.update(CP.HmassP_INPUTS, hot_ex.h, hot_ex.p)
        hot_ex.set_T(self.AS_H.T())
