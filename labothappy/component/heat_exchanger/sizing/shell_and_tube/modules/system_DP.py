...
class ShellAndTubeSizingOpt(BaseComponent):
    ...
    def evaluate_with_penalty(self, objective_function, particle, constraints, penalty_factor,i):
        ...
        score = objective_function(particle.HX.params)
        penalty = 0
                ...
                for opt_var in self.opt_vars:
                    ...
                    if opt_var in self.choice_vectors.keys():
                        ...
                        if opt_var == 'L_shell':
                            ...
                            r1 = np.random.rand()*self.bounds[opt_var][0]/10 #/3
                            r2 = np.random.rand()*self.bounds[opt_var][0]/10 #/3
                        else:
                ...
                for bound_key in self.bounds:
                    ...
                    if bound_key == 'L_shell':
                        ...
                        if self.particles[i].position[bound_key] < low_bound_L_shell:
                            ...
                            if high_bound_L_shell == self.bounds['L_shell'][0]:
    ...
    def opt_size(self):
        ...
        return self.particle_swarm_optimization(objective_function = self.HX_Mass , bounds = self.bounds, num_particles = 100, num_dimensions = len(self.opt_vars), max_iterations = 50, inertia_weight = 0.5,
...
# su_S = MassConnector()
# su_S.set_properties(T = 273.15 + 33.9, # K
#                     P = 2*1e5, # Pa
#                     m_dot = 22.07, # kg/s
#                     fluid = 'Water'
#                     )
...
# ex_S = MassConnector()
# ex_S.set_properties(T = 273.15 + 29.4, # K
#                     P = 1.5*1e5, # Pa
#                     m_dot = 22.07, # kg/s
#                     fluid = 'Water'
#                     )
...
# su_T = MassConnector()
# su_T.set_properties(T = 273.15 + 23.9, # K
...
# ex_T = MassConnector()
# ex_T.set_properties(T = 273.15 + 26.7, # K
#                     P = 1.5*1e5, # Pa
#                     m_dot = 35.31, # kg/s
#                     fluid = 'Water'
#                     )
...
# HX_test.set_constraints(Q_dot = 0.313*1e6, DP_h = 9.4*1e3, DP_c = 36.5*1e3)

"""
Parameters Setting
"""

best_scores = [2666.02026, 2655.080423141231, 2655.140774978429, 2655.0736391725895, 2582.4644551601436, 2654.6434877320366]
...
HX_test.set_parameters(
                        ...
                        foul_t = 0, # 0.0002, # 0.0002 # 0.00017 # (m^2 * K/W)
                        foul_s = 0,# 0.00033, # 0.00033 # 0.00017 # (m^2 * K/W)
                        tube_cond = 50, # W/(m*K)
                        Overdesign = 0,
...
#         particle_position['Central_spac'] = round(random.uniform(low_bound_central_spac, high_bound_central_spac),2)
#         particle_position['L_shell'] = round(self.random_multiple(lower_bound = max(self.bounds['L_shell'][0], particle_position['Shell_ID_inch']*(25.4/1000)*3), upper_bound = min(self.bounds['L_shell'][1], particle_position['Shell_ID_inch']*(25.4/1000)*15) , multiple = particle_position['Central_spac']),2)
#         particle_position['Baffle_cut'] = round(random.uniform(self.bounds['Baffle_cut'][0], self.bounds['Baffle_cut'][1]),2)
...
#                     if bound_key == "L_shell":
#                         if self.particles[i].position[bound_key] < self.particle[i].position['Shell_ID_inch']*(25.4/1000)*3:
#                             self.particles[i].position[bound_key] = self.particle[i].position['Shell_ID_inch']*(25.4/1000)*3
#                             self.particles[i].velocity[bound_key] = -self.particles[i].velocity[bound_key]
#                             bound_flag = 1
                        
#                         if self.particles[i].position[bound_key] > self.particle[i].position['Shell_ID_inch']*(25.4/1000)*15:
#                             self.particles[i].position[bound_key] = self.particle[i].position['Shell_ID_inch']*(25.4/1000)*15
#                             self.particles[i].velocity[bound_key] = -self.particles[i].velocity[bound_key]
#                             bound_flag = 1 
...
#     TS_t = tube_sheet_thickness(D_o, 0.0187, 95+273.15, P_design , D_s)
#     Full_Tube_sheet_A = np.pi*(D_s/2)**2
#     Tube_in_tube_sheet_A = n_t*np.pi*(D_o/2)**2
