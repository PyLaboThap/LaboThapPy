...
class HeatExchangerMB(BaseComponent):
    ...
    def external_pinching(self):
        ...
        T_hmin = CP.PropsSI('Tmin', 'P', self.H_su.p,'Q', 0, self.H_su.fluid)+0.5
        T_cmax = CP.PropsSI('Tmax', 'P', self.C_su.p,'Q', 0, self.C_su.fluid)-0.5
        ...
        h_ho_ideal = CP.PropsSI('H','T', max(self.T_ci, T_hmin),'P',self.p_hi,self.H_su.fluid) # Equation 5 (Bell et al. 2015)
        ...
        h_co_ideal = CP.PropsSI('H','T',min(self.T_hi, T_cmax),'P',self.p_ci,self.C_su.fluid) # Equation  (Bell et al. 2015)
        ...
        for k in range(len(self.hvec_h)-1): # iterate over each cell
            ...
            if self.H.HeatExchange_Correlation == "User-Defined":
                ...
                # 2 PHASE CORRELATION
                    ...
                    if self.H_su.fluid == 'R1233zd(E)':
                        T_Q0 = CP.PropsSI('T', 'Q', 0, 'P', p_h_mean, self.H_su.fluid)
                        k_h_l = conducticity_R1233zd(T_Q0, p_h_mean)
                        mu_h_l = CP.PropsSI('V', 'Q', 0, 'P', p_h_mean, self.H_su.fluid)
                        cp_h_l = CP.PropsSI('CPMASS', 'Q', 0, 'P', p_h_mean, self.H_su.fluid)
            ...
            if self.C.HeatExchange_Correlation == "User-Defined":
                ...
                # 2 phases case
                    ...
                    if self.C_su.fluid == 'R1233zd(E)':
                        ...
                        cp_c_l = CP.PropsSI('CPMASS', 'Q', 0, 'P', p_c_mean, self.C_su.fluid)
