...
class PumpInterpolation(BaseComponent):
    """
    **Component**: Pump (based on intperolation)
    
    **Model**: Model creating a performance and NPSHr interpolators based on either
        - 1 design point.
        - 1 flow rate / height curve for the rated speed.
        - 1 map based on speed, height, and flowrate.
    ...
    **Parameters**:
        ...
        If one point is given:
            ...
            V_dot_rated: Rated isentropic efficiency [m]

            Delta_H_rated: Rated head rise [m]

            eta_is_rated: Rated isentropic efficiency [m]

            N_rot_rated: Rated pump speed [rpm]

        If one curve is given:
        ...
        If a whole map is given:
            
            V_dot_map: 2D Array of volumetric flowrate curve [m³/h]
    
            Delta_H_map: Corresponding array from head rise curve [m]
    
            eta_is_map: Corresponding array from isentropic efficiency curve [-]
    
            NPSH_r_map: Corresponding array from NPSH required curve [m]
    
            N_rot_map: Rated pump speed [rpm]
    ...
    **Outputs**:
    ...
    def __init__(self, info_level = "1P"):
        """
        info level : Level of provided information
            - 1P : For 1 design operating point
            - 1C : For a design speed operating curve
            - 1M : For a design map
        """
    ...
    def get_required_parameters(self):
        ...
        if self.info_level == "1P":
            return ["V_dot_rated", "Delta_H_rated", "eta_is_rated", "N_rot_rated", "mode"]

        if self.info_level == "1C":
            return ["V_dot_curve", "Delta_H_curve", "eta_is_curve", "NPSH_r_curve", "N_rot_rated", "mode"]
        
        if self.info_level == "1M":
            return ["V_dot_map", "Delta_H_map", "eta_is_map", "NPSH_r_map", "N_rot_map", "mode"]

    def construct_interpolation(self):
        
        if self.info_level == "1P":
            
            # Check if same fluid than reference : Water, or other
