...
class AxialTurbineMeanLine(BaseComponent):
    ...
    # =========================================================================
    #  Map generation  (unchanged — operates on snapshots, not live stage dicts)
    # =========================================================================

    def generate_map_m_dot_N_rot(
        self, m_dot_grid=None, N_rot_grid=None, *,
        m_dot_range=None, N_rot_range=None,
        fixed_P_su=None, fixed_T_su=None, fixed_P_ex=None,
        per_point_hook=None, max_retries=2,
        mach_limit=1.2, pressure_tol=0.02, verbose=False,
    ):
        import numpy as _np
        ...
        if m_dot_grid is None:
            ...
            m_dot_grid = _np.linspace(*(m_dot_range or (0.7*m0, 1.3*m0, 9)))
        else:
            m_dot_grid = _np.array(list(m_dot_grid), dtype=float)

        if N_rot_grid is None:
            N0 = self.inputs.get('N_rot', 1000.0)
            N_rot_grid = _np.linspace(*(N_rot_range or (0.6*N0, 1.2*N0, 9)))
        else:
            N_rot_grid = _np.array(list(N_rot_grid), dtype=float)

        _P_su0 = self.inputs.get('P_su')
        _T_su0 = self.inputs.get('T_su')
        _P_ex0 = self.inputs.get('P_ex')
        ...
        for N in N_rot_grid:
            for m in m_dot_grid:
                ...
                if converged:
                    try:
                        P_ex_calc    = float(self.stages[-1].static_states[2]['P'])
                        W_dot        = getattr(self, 'W_dot', _np.nan)
                        eta          = getattr(self, 'eta_is', _np.nan)
                        RP_target    = self.inputs['P_su'] / self.inputs['P_ex'] if self.inputs.get('P_ex') else _np.nan
                        machs        = []
                        for st in self.stages:
                            for label in ('M2_S', 'M3_R'):
                                val = getattr(st, label, None)
                                if val is not None:
                                    machs.append(val)
                        mach_warn     = any(_np.isfinite(x) and x > mach_limit for x in machs)
                        pressure_warn = False
                        if self.inputs.get('P_ex') and _np.isfinite(P_ex_calc):
                            rel_err       = abs(P_ex_calc - self.inputs['P_ex']) / self.inputs['P_ex']
                ...
                rows.append(dict(
                    m_dot         = float(m),
                    N_rot         = float(N),
                    P_su          = float(self.inputs.get('P_su', _np.nan)),
                    T_su          = float(self.inputs.get('T_su', _np.nan)),
                    P_ex_target   = float(self.inputs.get('P_ex', _np.nan)),
                    P_ex_calc     = float(P_ex_calc) if _np.isfinite(P_ex_calc) else _np.nan,
                    PR            = float(RP_target)  if _np.isfinite(RP_target)  else _np.nan,
        ...
        self.set_inputs(
            m_dot = self.inputs['m_dot'], P_su = _P_su0,
            T_su  = _T_su0, N_rot = self.inputs['N_rot'],
            fluid = self.fluid, P_ex = _P_ex0,
        )
