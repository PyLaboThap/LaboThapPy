...
class CompressorSE(BaseComponent):
    ...
    **Ouputs**:
    ...
    def print_setup(self):
        ...
        print(
            f"  - Q_amb: T_cold={self.Q_amb.T_cold}, T_hot={self.Q_amb.T_hot}, Q_dot={self.Q_amb.Q_dot}"
        )
...
<<<<<<< HEAD:labothappy/component/volumetric_machine/compressor/steady_state/semi_empirical/simulation_model.py
        s_ex2_bis = PropsSI(
            "S", "T", self.T_ex2, "P", self.P_ex2, Fluid
        )  # C ICI LE PB??
...
=======
        ...
        try: 
            cv_leak = PropsSI('CVMASS','P', self.P_ex2, 'H', h_ex2_bis, Fluid)
            cp_leak = PropsSI('CPMASS','P', self.P_ex2, 'H', h_ex2_bis, Fluid)
>>>>>>> f3c1eeec131999afb63fd3a31ec25f2fd9087f48:labothappy/component/volumetric_machine/compressor/steady_state/compressor_SE.py
        except:
        ...
        h_su2 = max(
            min((self.m_dot * h_su1 + m_dot_leak * h_ex2_bis) / m_dot_in, h_ex2_bis),
            h_su1,
        )
        ...
        v_thr_ex = 1.0 / PropsSI("D", "P", P_thr_ex, "S", s_ex2, Fluid)
        ...
        W_dot_loss = (
            self.params["alpha"] * W_dot_in
            + self.params["W_dot_loss_0"]
            + self.params["C_loss"] * self.N * 2 * np.pi
        )
        ...
        self.resE = abs(
            (W_dot_loss - Q_dot_ex - Q_dot_su - self.Q_dot_amb) / (W_dot_loss)
        )
    ...
    def solve(self):
        ...
        if not (self.calculable and self.parametrized):
            self.solved = False
            print(
                "CompressorSE could not be solved. It is not calculable and/or not parametrized"
            )
            return
...
<<<<<<< HEAD:labothappy/component/volumetric_machine/compressor/steady_state/semi_empirical/simulation_model.py
            ...
            x_m_guess = [
                ...
                1,
                1.15,
            ]  # guesses on the filling factor to provide suitable initial point for the iteration
...
>>>>>>> f3c1eeec131999afb63fd3a31ec25f2fd9087f48:labothappy/component/volumetric_machine/compressor/steady_state/compressor_SE.py
            ...
            while not stop and j < len(x_T_guess):
                ...
                while not stop and k < len(x_m_guess):
                    ...
                    T_w_guess = (
                        x_T_guess[j]
                        * PropsSI("T", "P", self.ex.p, "S", self.su.s, self.su.fluid)
                        + 5
                    )  # x_T_guess[j]*self.su.T+(1-x_T_guess[j])*self.T_amb
                    m_dot_guess = (
                        x_m_guess[k]
                        * self.params["V_s"]
                        * self.inputs["N_rot"]
                        / 60
                        * self.su.D
                    )  # ff_guess[k]*self.V_s*self.N_rot/60*PropsSI('D', 'P', self.su.p, 'H', self.su.h, self.su.fluid) #initial value for M_dot
                    T_ex2_guess = (
                        PropsSI("T", "P", self.ex.p, "S", self.su.s, self.su.fluid) + 5
                    )  # PropsSI('T', 'P', self.su.p*self.rp,'S', self.su.s, self.su.fluid)
