...
class PumpExtrapolationModelP_out(BaseComponent):
    def __init__(self):
...
#%%
    def get_required_inputs(self): # Used in check_calculablle to see if all of the required inputs are set
        ...
        return ['su_p', 'su_T', 'su_m_dot', 'su_fluid', 'Omega_pp']
...
#%%
    ...
    def solve(self):
        ...
        if self.su.m_dot < 0 or self.Omega_pp < 0: 
            ...
            V_DH = interp1d(self.V_dot_curve, self.DH_curve, kind='linear', fill_value='extrapolate')
            ...
            "Height Difference"
            self.DH = V_DH(self.su.V_dot)
            
            "Pressure Difference"
            DP = self.DH*(g*self.su.D)
            
            "Exhaust Pressure"
            self.p_ex = self.su.p + DP
            
            if self.su.V_dot > self.V_dot_curve[-1] or self.su.V_dot < self.V_dot_curve[0]:
                self.V_dot_flag = 1
            ...
            self.eta_is = V_eta(self.su.V_dot)
            
            "NPSH_r"
            self.NPSH_r = V_NPSH_r(self.su.V_dot)
            ...
            self.eta_el = self.eta_el_fun(100*(self.W_dot_wf/(self.params['eta_m']*self.eta_is)) / self.params['W_dot_el_rated'])/100
