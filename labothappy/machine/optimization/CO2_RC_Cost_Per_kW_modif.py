...
def system_RC_parallel(x, input_data):
    ...
    try:
        ...
        return 10000.0, np.inf, np.nan


def system_RC_parallel_return(x, input_data):
    warnings.filterwarnings('ignore')
    ...
    try:
        ...
        return 1000.0, np.inf, np.nan, RC
...
class CO2RCOptimizer:
    ...
    def opt_RC(self, n_jobs=None, n_particles=30, max_iter=30, patience=10, tol=1e-4, ntop = 10):
        ...
        def objective_wrapper(X):
            ...
            for x_i, pen_i, cost in zip(X, penalties, costs):
                ...
                if pen_i <= 10 and np.isfinite(cost):
    ...
    def cycle_design(self, n_jobs=None, n_particles=30, max_iter=30, patience=10, tol=1e-4, ntop = 5):
        ...
        self.best_cost, self.best_pen, self.best_eta, self.best_RC = system_RC_parallel_return(self.top_positions[0]["x"], self.input_data)
...
if __name__ == "__main__":
    ...
    m_dot_bounds = np.array([20,50])*n_MW
