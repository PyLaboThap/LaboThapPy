...
# ── Worker-local lazy cache ────────────────────────────────────────────────────
_WORKER_MAP_DF = None

def _get_worker_map():
    global _WORKER_MAP_DF
    if _WORKER_MAP_DF is None:
        import pandas as pd
        _WORKER_MAP_DF = pd.read_parquet(MAP_SAVE_PATH)
    return _WORKER_MAP_DF
...
def CO2_OD_TC(mdot_CO2_guess, P_high_guess, SC_target, T_su_w_cd, m_dot_w_cd, N_pp, N_exp, damping_it, mute_print=1):
...
if __name__ == "__main__":
    ...
    N_POINTS = 8
    N_pp_sweep  = np.linspace(0.85 * N_PP_NOMINAL,  1.15 * N_PP_NOMINAL,  N_POINTS)
