...
#     Turb.sizing(max_iter=10, n_jobs=-1)
...
def _eval_particle(x, cls, fluid, params, inputs):
    ...
    Retourne aussi `debug` (le dict rempli par design_system._record_debug),
    en plus de `cost` et `W_dot` -- permet à sizing() de construire
    self.eval_log pour tracer obj vs itération PSO par particule.
    """
    os.environ.setdefault("OMP_NUM_THREADS", "1")
    os.environ.setdefault("MKL_NUM_THREADS", "1")
    os.environ.setdefault("OPENBLAS_NUM_THREADS", "1")
    ...
    return cost, float(_SOLVER.W_dot), debug
...
class RadialTurbineMeanLineSizing(object):
    ...
    # Bornes par défaut pour l'optimisation PSO (design_system / opt_size / sizing).
    #
    # eta_is_guess_bounds (NOUVEAU) : borne la 6e dimension du PSO, qui
    # remplace le calcul rigide Dh0 = W_dot/mdot -- voir design_system.
    # C'est une fraction de la chute isentropique disponible (Dh0s), donc
    # bornée dans [0,1] par construction physique, peu importe l'échelle
    # absolue du problème (contrairement à un W_dot brut). Mêmes bornes que
    # pour la turbine axiale par défaut ; à ajuster si l'expérience montre
    # que la turbine radiale plafonne à un niveau différent.
    DEFAULT_BOUNDS = {
        'r5_r4_bounds': [0.3, 0.7],     # [-] : r5/r4 ratio
        'psi_bounds': [0.5, 1.5],
        'phi_bounds': [0.3, 0.6],
    ...
    def __init__(self, fluid):
        ...
        # NOUVEAU : valeurs par défaut sûres, pour que même un échec de
        # design_system() en fin de sizing() (ré-évaluation de best_pos)
        # affiche nan proprement plutôt que de planter avec un
        # AttributeError sur un attribut jamais défini sur CET objet
        # (design_system tourne dans des workers séparés pendant le PSO --
        # self.Omega/self.eta_is/self.W_dot n'existent sur l'objet
        # principal qu'après un appel RÉUSSI de design_system dessus).
        self.Omega = float('nan')
        self.eta_is = float('nan')
        self.eta_is_guess = float('nan')
        self.W_dot = 0.0
        self.params.setdefault('r4', float('nan'))
        self.params.setdefault('r5', float('nan'))
    ...
    def export_params_dict(self):
        ...
        return {
            ...
            "eta_is_guess": self.eta_is_guess,
            "N_rot_rated": self.Omega,
            "total_to_static_efficiency": self.eta_is,
            "DP_rated": round(self.inputs['p0_su']/self.inputs['p_ex'],2),
            "n_stages": 1,
    ...
    def _record_debug(self, x):
        ...
        soit faisable ou non -- sert à tracer obj vs itération PSO par
        particule (voir sizing() -> objective_wrapper -> self.eval_log),
        même quand design_system échoue tôt (avant que eta_is, r4, r5,
        Omega... existent).

        getattr(...)/dict.get(...) partout : selon À QUEL POINT
        design_system a échoué (TimeoutError/except très tôt, avant même
        que designRotor() ait tourné), la plupart des attributs peuvent ne
        pas encore exister ou ne pas avoir été mis à jour pour CETTE
        évaluation. eta_is_guess, lui, est fixé tout au début de
        design_system, donc quasi toujours disponible.
        """
        self._debug_record = {
            "obj": getattr(self, "obj", float("nan")),
            "eta_is": getattr(self, "eta_is", float("nan")),
            "eta_is_guess": getattr(self, "eta_is_guess", float("nan")),
            "Omega": getattr(self, "Omega", float("nan")),
            "r4": self.params.get("r4", float("nan")),
            "r5": self.params.get("r5", float("nan")),
            "psi": self.inputs.get("psi", float("nan")),
            "phi": self.inputs.get("phi", float("nan")),
            "xhi": self.inputs.get("xhi", float("nan")),
            "r5_r4_ratio": self.params.get("r5_r4_ratio", float("nan")),
            "r5h_r5t_ratio": self.params.get("r5h_r5t_ratio", float("nan")),
    ...
    def design_system(self, x):    
        ...
        # (self.inputs['W_dot']/self.inputs['mdot']), qui imposait une
        # valeur fixe sans lien garanti avec ce que la géométrie peut
        # réellement délivrer en retombant sur p_ex -- c'était aussi la
        # cause du crash observé (le cliquet W_dot faisait dériver la
        # cible entre l'évaluation de best_pos pendant le PSO et sa
        # ré-évaluation finale sur self, menant à un échec silencieux).
        # eta_is_guess est désormais la 6e dimension du PSO (x[5]) : une
        # fraction de la chute isentropique disponible Dh0s, bornée par
        # construction dans [0,1] (voir eta_is_guess_bounds) -- même
        # logique que pour la turbine axiale.
        self.eta_is_guess = x[5]
        self.Dh0 = self.eta_is_guess * Dh0s
...
#%%
    ...
    def opt_size(self):
        ...
        for i in range(max_iter):
            ...
            # NOTE : le cliquet W_dot qui existait ici a été retiré -- il
            # est devenu du code mort de toute façon, puisque
            # design_system() ne lit plus self.inputs['W_dot'] pour
            # calculer Dh0 (remplacé par eta_is_guess = x[5]). C'était
            # aussi la cause du crash observé : la cible dérivait entre
            # l'évaluation de best_pos pendant le PSO et sa ré-évaluation
            # finale, menant à un échec silencieux de design_system().
        ...
        best_pos = optimizer.swarm.best_pos
        final_cost = self.design_system(best_pos)
        if final_cost >= 1000:
            print("⚠️  Le design final (best_pos) échoue à la ré-évaluation -- "
                  "résultats potentiellement incomplets/nan ci-dessous.")
    ...
    def sizing(self, n_jobs=-1, backend="loky", chunksize="auto", n_particles = 20, max_iter=50):
        ...
        objective_wrapper -> _eval_particle -> design_system._record_debug().
        Réinitialisée à CHAQUE appel de sizing().
        ...
        pour calculer Dh0 (remplacé par eta_is_guess = x[5], voir
        design_system). C'était la cause du crash "AttributeError: no
        attribute 'Omega'" : la cible dérivait entre l'évaluation de
        best_pos DANS LE PSO (sous un W_dot donné) et sa ré-évaluation
        finale sur self (sous le W_dot ratcheté, potentiellement plus
        élevé), menant à un échec silencieux (obj=1000, sans que Omega/
        eta_is/W_dot ne soient jamais définis sur cet objet). self.inputs
        ['W_dot'], s'il est fourni, n'est conservé qu'à titre informatif.
        """
        os.environ["PYTHONWARNINGS"] = "ignore" 
        ...
        # remplacé par eta_is_guess = x[5]) -- conservé ici uniquement si
        # l'appelant le fournit, à titre informatif.
        inputs_snapshot = {
            "p0_su": pick("p0_su", "P0_su", "P_su", "p_su"),
            "T0_su": pick("T0_su", "t0_su", "T_su", "t_su"),
            "p_ex" : pick("p_ex", "P_ex"),
        ...
        self.eval_log = []       # <-- reset à chaque appel
        ...
        def objective_wrapper(X):
            ...
            # results: list of (cost, W_dot, debug)
            costs = [r[0] for r in results]
            wdots = [r[1] for r in results]

            for particle_idx, (c, w, debug) in enumerate(results):
        ...
        for i in range(max_iter):
            ...
            # NOTE : le cliquet W_dot qui existait ici (raise du target
            # entre itérations) a été retiré -- voir docstring ci-dessus.
        ...
        # Finalize -- best_pos est ré-évalué sur self (design_system tourne
        # dans des workers séparés pendant le PSO) : puisque le cliquet
        # W_dot a disparu, self.inputs['W_dot'] (informatif seulement, non
        # utilisé par design_system) n'a plus dérivé entre-temps, donc
        # cette ré-évaluation retombe sur EXACTEMENT le même problème que
        # celui optimisé -- elle ne devrait donc (quasi) plus jamais
        # échouer là où le cliquet la faisait échouer avant.
        final_cost = self.design_system(best_pos)
        if final_cost >= 1000:
            print("⚠️  Le design final (best_pos) échoue à la ré-évaluation -- "
                  "résultats potentiellement incomplets/nan ci-dessous.")
